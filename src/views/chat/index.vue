...
import {fetchChatAPIProcess, fetchSession} from '@/api'
...
function handleSubmit() {
    if(userStore.userInfo.balance <= 0){
        ms.error("您的余额已不足")
        return
    }
    onConversation()
...
function calcTextLength(messages: any[]) {
    let length = 0
...
async function onConversation() {
    ...
    const chatLen = calcTextLength(conversationList.value);
    ...
    if (lastContext && usingContext.value) {
        if (chatLen > 1000) {
            options = {...lastContext, parentMessageId: undefined}
            ms.info("上下文超出指定长度 1000 个字符串，已为您重新开启新的上下文")
        } else {
            options = {...lastContext}
        }
    }
